...
// int rehashing(int** hash_table, int size){
//   curr_size = 0;
//   size*=2;
//   int* temp = *hash_table;
...
int rehashing(int** hash_table, int m, int size){
  curr_size = 0;
  int prev=size;
  int* prev_hash = * hash_table;
  size*=2;
  int* temp;
  temp = realloc(*hash_table, size * sizeof(int));

  init_hash_table(temp, size);
  for(int i=0; i<prev; i++){
    if(prev_hash[i]!=-1){
    printf("%d prev hash\n", prev_hash[i]);
    insert_using_linear_probing(prev_hash[i], m, temp, size);
    }
  }
  return size; 
}
...
int insert_using_linear_probing(int key, int m, int* table, int size){
  ...
  printf("HASH for %d  is  %d and size is %d\n\n", key, hash, size);
  if(i==size-1){
    return 0;
  }
  table[hash] = key;
